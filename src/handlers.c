...
static esp_err_t ota_send_file(uint8_t *file, size_t length)
{
    esp_ble_ota_write(file, length);
    return ESP_OK;
}

static esp_err_t ota_start_cmd(size_t file_size, size_t block_size, char *partition_name)
{
...
static esp_err_t ota_finish_cmd()
{
    esp_ble_ota_finish();
    return ESP_OK;
}
