...
typedef struct ota_cmd {
    int cmd_id;
    esp_err_t (*command_handler)(SendFilePayload *req, SendFilePayload *resp, void *priv_data);
} ota_cmd_t;

static esp_err_t ota_send_file_handler(SendFilePayload *req, SendFilePayload *resp, void *priv_data);

static esp_err_t ota_start_cmd_handler(SendFilePayload *req, SendFilePayload *resp, void *priv_data);

static esp_err_t ota_finish_cmd_handler(SendFilePayload *req, SendFilePayload *resp, void *priv_data);

static ota_cmd_t cmd_table[] = {
    {.cmd_id = SEND_FILE_MSG_TYPE__TypeCmdSendFile, .command_handler = ota_send_file_handler},
    {.cmd_id = SEND_FILE_MSG_TYPE__TypeCmdStartOTA, .command_handler = ota_start_cmd_handler},
    {.cmd_id = SEND_FILE_MSG_TYPE__TypeCmdFinishOTA, .command_handler = ota_finish_cmd_handler}};
...
static esp_err_t ota_send_file_handler(SendFilePayload *req, SendFilePayload *resp, void *priv_data)
{
    ...
    resp->status         = (h->ota_send_file(req->cmd_send_file->data.data, req->cmd_send_file->data.len) == ESP_OK
                                ? STATUS__Success
...
static esp_err_t ota_start_cmd_handler(SendFilePayload *req, SendFilePayload *resp, void *priv_data)
{
    ...
    resp->status         = (h->ota_start_cmd(req->cmd_start_ota->file_size, req->cmd_start_ota->block_size,
...
static esp_err_t ota_finish_cmd_handler(SendFilePayload *req, SendFilePayload *resp, void *priv_data)
{
    ...
    resp->status          = (h->ota_finish_cmd() == ESP_OK ? STATUS__Success : STATUS__InternalError);
...
static void ota_cmd_cleanup(SendFilePayload *resp, void *priv_data)
{
    switch (resp->msg) {
...
static esp_err_t ota_cmd_dispatcher(SendFilePayload *req, SendFilePayload *resp, void *priv_data)
{
    esp_err_t ret;
